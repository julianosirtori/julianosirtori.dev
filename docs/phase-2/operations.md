...
O Resend de Production também está preparado: domínio verificado, segmentos PT/EN, chave própria e segredos na Vercel. O webhook está desativado até o deploy da rota. O plano limita a conta a três segmentos; o proprietário optou por manter o plano e deixar a newsletter de Preview pendente. O domínio `preview.julianosirtori.dev` está vinculado à branch do PR, com proteção Vercel, e seu CNAME na Cloudflare foi validado pela Vercel. Ainda falta confirmar a disponibilidade HTTPS com o deploy de Preview. Recursos, cotas configuradas e passos restantes estão no [guia Resend](resend.md). Ainda faltam GitHub OAuth, validação real dos fluxos e GA4. Não houve deploy nem envio real de e-mail durante essa configuração.
...
Na configuração real de Production, os limites foram reduzidos para **60/dia e 1.800/mês na newsletter**, mais **10/dia e 300/mês no contato**, reservando capacidade para outros usos da conta compartilhada. Preview tem **2/dia e 20/mês para contato**, com a newsletter pendente pela cota de segmentos. Esses valores na Vercel substituem os padrões acima após o próximo deploy.
