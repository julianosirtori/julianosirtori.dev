...
Como podemos ver acima, fazemos um filtro no nosso array mergeado, passando um callback que verifica se o índice do primeiro aparecimento do `item` no array `all` é igual ao índice atual (`index`) do loop. Dessa forma, o callback retorna `true` apenas para a primeira ocorrência do `item`.
